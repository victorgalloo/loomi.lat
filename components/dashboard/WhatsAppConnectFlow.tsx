    ...
    try {
      ...
      if (data.type === 'WA_EMBEDDED_SIGNUP') {
        ...
        if (signupEvent.data.phone_number_id && signupEvent.data.waba_id) {
          // Store signup data for the callback
          const signupData = {
          ...
          // Data will be sent to backend when the OAuth callback fires
        }
  ...
  if (success) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
        ...
        <h3 className="text-xl font-semibold text-green-800 mb-2">WhatsApp Conectado</h3>
        {success.businessName && (
          <p className="text-green-700">{success.businessName}</p>
        )}
        {success.phoneNumber && (
          <p className="text-green-600">{success.phoneNumber}</p>
        )}
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-8">
      ...
      <button
        ...
        className={`
          flex items-center gap-3 px-8 py-4 rounded-xl font-medium text-white text-lg
          transition-all duration-200
          ${
            isSDKLoaded && !isLoading && !isConnecting
              ? 'bg-[#25D366] hover:bg-[#128C7E] cursor-pointer shadow-lg hover:shadow-xl'
      ...
      {/* Instructions */}
      <div className="max-w-md text-center text-sm text-gray-500 space-y-2">
        <p>Al hacer clic, se abrira una ventana de Meta donde podras:</p>
        <ul className="text-left list-disc list-inside space-y-1">
          <li>Crear una cuenta de WhatsApp Business API</li>
          <li>Verificar tu numero de telefono</li>
          <li>Autorizar a Loomi para enviar mensajes</li>
        </ul>
      </div>
    </div>
  );
